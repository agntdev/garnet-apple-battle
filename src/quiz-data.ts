...
export function displayName(ctx: Ctx): string { const from = ctx.from; return from ? ([from.first_name, from.last_name].filter(Boolean).join(" ") || "Participant") : "Participant"; }
export function secureToken(): string { return crypto.randomUUID().replace(/-/g, "").slice(0, 20); }
export function publicQuestion(question: QuizQuestion, number: number): string { return `Question ${number} of ${QUESTION_COUNT} — ${QUESTION_SECONDS} seconds\n\n${question.prompt}`; }
...
export function resultsCsv(session: QuizSession): string { const cell = (v: string | number) => `"${String(v).replace(/"/g, '""')}"`; return ["Rank,Participant,Score,Total response time (seconds)", ...rankedParticipants(session).map((p, i) => [i + 1, p.display_name, p.score, (p.total_response_time / 1000).toFixed(2)].map(cell).join(","))].join("\n"); }
