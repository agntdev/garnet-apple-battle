...
import { readQuizState, secureToken, type QuizQuestion, writeQuizState } from "../quiz-data.js";
...
async function promptForQuestion(ctx: Ctx, edit: boolean) {
  ...
  // The stored bank is authoritative.  This makes reopening the flow safe even
  // after a worker restart and prevents overwriting an existing question.
  const number = state.template.length + 1;
  ctx.session.quizSetup = { nextQuestionNumber: number };
...
async function showManage(ctx: Ctx, edit: boolean) {
  ...
  const text = live
    ...
    : state.template.length > 0
      ? RU.manageReady(state.template.length)
      : RU.manageSetup;
  const markup = live
...
async function startQuiz(ctx: Ctx) {
  ...
  if (state.template.length === 0) { await ctx.reply(RU.addQuestionsBeforeStarting, { reply_markup: setupKeyboard }); return; }
  // Snapshot the complete persisted bank. Later additions belong to a future
  // session, while every question present at start is used in this session.
...
composer.callbackQuery("quiz:setup", async (ctx) => { await ctx.answerCallbackQuery(); if (!(await requireOwner(ctx, ownerMessages))) return; await promptForQuestion(ctx, true); });
composer.callbackQuery("quiz:setup:next", async (ctx) => { await ctx.answerCallbackQuery(); if (!(await requireOwner(ctx, ownerMessages))) return; await promptForQuestion(ctx, true); });
